...
namespace hardware {
    ...
    void Motor::update_feedback(Coms connection_) {
        ...
        if (connection_.read2ByteTxRx(id_, RAMAddresses::PRESENT_SPEED, &motor_byte_velocity_)) {
            if (motor_byte_velocity_ == MIN_CCW_MOTOR_RPM_BYTE || motor_byte_velocity_ == MIN_CW_MOTOR_RPM_BYTE) {
                ...
                motor_rpm_ = interpolate(MIN_CCW_MOTOR_RPM_BYTE, MIN_RPM, MAX_CCW_MOTOR_RPM_BYTE, MAX_RPM,
                                         motor_byte_velocity_);
                actual_velocity_ = rpm_to_rad_sec(motor_rpm_);
            } else if (motor_byte_velocity_ <= MAX_CW_MOTOR_RPM_BYTE) {
                // might have to change if statements to add lower bound
                motor_rpm_ = -interpolate(MIN_CW_MOTOR_RPM_BYTE, MIN_RPM, MAX_CW_MOTOR_RPM_BYTE, MAX_RPM,
                                          motor_byte_velocity_);
                actual_velocity_ = rpm_to_rad_sec(motor_rpm_);
            }
    ...
    void Motor::go_home(Coms connection_) const {
        util::LoggerManager::getLogger()->info("Going home");
        connection_.write2ByteTxRx(id_, RAMAddresses::GOAL_POSITION, home_byte_position);
    }
