...
namespace hardware {
    ...
    void Motor::enable_torque() const {
        if (!connection_->write1Byte(id_, RAMAddresses::TORQUE_ENABLE, ENABLE)) {
    ...
    void Motor::disable_torque() const {
        if (!connection_->write1Byte(id_, RAMAddresses::TORQUE_ENABLE, DISABLE)) {
    ...
    bool Motor::is_torque_enabled() const {
        if (connection_->read1Byte(id_, RAMAddresses::TORQUE_ENABLE, torque_status_) == ENABLE) {
            return true;
        }
        return false;
    }
     ...
     */
    bool Motor::set_commanded_position(const double_t commanded_position) {
        ...
        if (!connection_->write2Byte(id_, RAMAddresses::GOAL_POSITION, motor_byte_position_command_)) {
    ...
    bool Motor::set_commanded_velocity(const double_t commanded_velocity) {
        ...
        if (!connection_->write2Byte(id_, RAMAddresses::MOVING_SPEED, motor_byte_rpm_command_)) {
    ...
    double_t Motor::get_feedback_position() {
        if (connection_->read2Byte(id_, RAMAddresses::PRESENT_POSITION, &motor_byte_position_)) {
    ...
    double_t Motor::get_feedback_velocity() {
        if (connection_->read2Byte(id_, RAMAddresses::PRESENT_SPEED, &motor_byte_velocity_)) {
